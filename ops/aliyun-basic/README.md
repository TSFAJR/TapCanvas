...
基础 Compose 还显式设置 `UPSTREAM_CATALOG_SYNC_ENABLED=false`，关闭 New API 启动时的商业目录同步。官方同步逻辑要求种子渠道启用，否则会中止启动；禁用同步使无模型部署能够正常启动，并记录明确日志。其他部署缺省保持官方同步行为，非法布尔值仍报错。以后需要同步时配置自己的渠道并明确开启该选项。
