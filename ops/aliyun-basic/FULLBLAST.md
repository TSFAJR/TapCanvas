...
Secrets live in `/etc/tap-canvas/runtime.env`. Set `FULLBLAST_API_KEY`, `TAPCANVAS_BASIC_NO_MODELS=0`, `TAPCANVAS_GENERATION_ENABLED=1`, `UPSTREAM_CATALOG_SYNC_ENABLED=false`, `AGENTS_MODEL=qwen3.7-plus`, `AGENTS_CAPABILITY_ANALYSIS_MODEL=qwen3.7-plus`, `DSH_CONTEXT_WINDOW=200000`, and `LOCAL_ASSET_PUBLIC_BASE_URL=http://47.112.1.188/assets/local`. Keep the existing internal gateway token. Channel credentials are also stored by New API in its private database and protected backups.
