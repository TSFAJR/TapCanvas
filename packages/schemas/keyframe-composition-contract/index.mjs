...
function readBoundedText(value, path, max, issues) {
    ...
    if (text.length > max)
        issues.push(`${path} 最多 ${max} 字（收到 ${text.length}）`);
    return text.slice(0, max);
}
function readEnum(value, values, path, issues) {
    if (typeof value === "string" && values.includes(value))
        return value;
    issues.push(`${path} 必须 ∈ {${values.join("/")}}`);
    return null;
}
function readPoint(value, path, issues) {
...
function readUniqueNames(value, path, issues) {
    if (!Array.isArray(value) || value.length === 0) {
        issues.push(`${path} 必须是至少一项的字符串数组`);
        return [];
    }
    if (value.length > 24)
        issues.push(`${path} 最多 24 项（收到 ${value.length}）`);
    ...
    if (new Set(valid).size !== valid.length)
        issues.push(`${path} 不允许重复名称`);
    return [...new Set(valid)];
}
function parseSubject(value, index, issues) {
    ...
    const visualWeight = readEnum(record.visualWeight, KEYFRAME_VISUAL_WEIGHTS, `${path}.visualWeight`, issues);
    const depthLayer = readEnum(record.depthLayer, KEYFRAME_DEPTH_LAYERS, `${path}.depthLayer`, issues);
    const centerPlacement = readEnum(record.centerPlacement, KEYFRAME_CENTER_PLACEMENTS, `${path}.centerPlacement`, issues);
    const maxFrameHeightRatio = Number(record.maxFrameHeightRatio);
    if (!Number.isFinite(maxFrameHeightRatio) ||
        maxFrameHeightRatio < 0.05 ||
        maxFrameHeightRatio > 1) {
        issues.push(`${path}.maxFrameHeightRatio 必须是 [0.05,1] 内有限数值`);
    }
    if (!name || !visualWeight || !depthLayer || !centerPlacement)
        return null;
    if (maxFrameHeightRatio < 0.05 || maxFrameHeightRatio > 1)
        return null;
...
export function hashKeyframeCompositionContract(contract) {
    return createHash("sha256").update(canonicalContractJson(contract)).digest("hex");
}
export function doesCompositionImageUrlCarryHash(imageUrl, hash) {
    if (!hash)
        return false;
    try {
...
export function parseKeyframeCompositionContract(value) {
    const issues = [];
    const record = readRecord(value);
    if (!record)
        return { ok: false, issues: ["compositionContract 必须是对象"] };
    const narrativeTask = readBoundedText(record.narrativeTask, "compositionContract.narrativeTask", 240, issues);
    const focusKind = readEnum(record.focusKind, KEYFRAME_FOCUS_KINDS, "compositionContract.focusKind", issues);
    const focusTargetNames = readUniqueNames(record.focusTargetNames, "compositionContract.focusTargetNames", issues);
    const focalPoint = readPoint(record.focalPoint, "compositionContract.focalPoint", issues);
    const shotScale = readEnum(record.shotScale, KEYFRAME_SHOT_SCALES, "compositionContract.shotScale", issues);
    const environmentVisualWeight = readEnum(record.environmentVisualWeight, KEYFRAME_VISUAL_WEIGHTS, "compositionContract.environmentVisualWeight", issues);
    if ((!Array.isArray(record.subjects) || record.subjects.length === 0)
        && focusKind !== "environment") {
        issues.push("compositionContract.subjects 在非环境焦点构图中必须至少包含一个角色");
    }
...
export function renderKeyframeCompositionFacts(contract) {
    const subjectFacts = contract.subjects
        .map((subject) => `${subject.name}=${subject.visualWeight}/${subject.depthLayer}/center:${subject.centerPlacement}/maxHeight:${subject.maxFrameHeightRatio}`)
        .join("；");
    return [
