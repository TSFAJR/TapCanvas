...
## 18. Lluban 启动同步与模型售价倍率

正常启动在初始化数据库与 Option 后、构建定价缓存和监听 HTTP 前，同步
`lluban-recommended` / `tag=lluban` 渠道。`UPSTREAM_CATALOG_CHANNEL_ID` 可显式指定
来源渠道；没有指定且没有 lluban 渠道时不触发同步，匹配多个渠道时报错要求指定 ID。
`--migrate-only` 不执行同步。同步失败会阻止启动，不把静态快照当作同步成功。

同步读取渠道 base URL 下 `/api/models/list?enabled=true` 与 `/api/pricing`，按上游
启用目录和公开可执行价格更新模型、参数、协议绑定与能力。价格来源是上游公开售价，
并非供应商内部采购成本；规格采用公开 `param_pricing`，不会复制可能尚未包含上游
渠道调价的原始 `pricing_config`。不支持的协议、不一致的线性规格价格、重复价格或
缺失定价会明确失败。同步事务保留渠道凭据；移除的模型如仍被其他启用渠道使用则保留价格。

模型管理 → 编辑模型 → 统一定价，对同步模型展示上游原价、售价倍率和售价预览。
`PUT /api/models/:id/pricing` 提交 `{ "selling_multiplier": 1.5 }` 即可保存。
倍率必须是正有限数字，首次导入为 1。token 输入基础倍率、按次价、规格固定价、
每秒价及参考图附加价均乘该倍率；token 输出、缓存、图片和音频的相对比例保持不变。
最终价格仍遵循系统已有用户分组计费规则。

原价、来源渠道、上游版本及倍率以 `options.UpstreamModelPricing:<model_name>` 保存，
无需新增表或字段。每次同步先读取保存的倍率，再从本次上游原价推导售价，不会重复乘倍率。
保存倍率与生效价格使用同一事务，提交后刷新定价缓存；数据库保存成功但刷新失败会明确报告。
同步模型不允许直接覆盖基础价；批量定价编辑也不能绕过倍率覆盖它们。

同步渠道的 OpenAI 协议绑定显式设置 `image_size_transport=passthrough`。
图片请求的 `size` 按上游契约原样传递，例如 lluban 使用 `16:9`；OpenAI adaptor
不得再因 `gpt-image-2` 模型名前缀将其改为 `1536x864`。分辨率档位仍独立传递，
不将 `1K/2K/4K` 填入比例字段。该选项同时适用于生成与 JSON 编辑请求。
