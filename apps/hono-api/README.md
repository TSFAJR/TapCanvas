...
- 章节画布按钮（资产图、镜头设计板、角色卡、衍生视频等）统一经 `dispatchIntent -> chatCommandStore -> AiChatDialog.send` 进入小T主对话。按钮只提交用户明确选择的目标、真实 project/book/chapter/sourceNode 事实、图片模型/分辨率、变体参数、风格和附加要求；`canvasNodeId` 锚定点击的源节点，不能被当前选中态替换。语言模型、会话、排队、补充选项、trace、durable transport 与 `expectedDelivery -> deliveryEvidence -> deliveryVerification` 均复用主对话。前端不再创建独立 `chapter-intent` 会话，不直调另一条 SSE、不维护独立进度 toast、导航终止守卫或补充选项弹窗；原 `streamChapterIntent` / `intentLifecycle` 已移除。剧本到资产库只派发一次完整用户目标，由 agents 自主选择技能和工具顺序；提交命令不等于生成完成，浏览器不自行宣告资产已就绪。生成的真实节点与媒体仍由服务端/画布持久事实承载，不回滚、不覆盖、不丢弃已成功资产。
- 画布与组节点入口复用 `chatCommandStore -> AiChatDialog -> agentsChatStream -> agents-cli`。组节点的一键成片v1只声明“按字长拆分并保存待生成视频节点”的用户交付意图，不强制加载 Skill。API 启动在管理员初始化后执行 `syncBuiltInOneClickWorkflow`，以同一版本化定义发布系统级 `all_users` attachment，所有新用户无需复制管理员私有画布即可装载/使用。真实 Workflow IR 为触发器 → 读取调用者文本组 → JavaScript 按 Unicode 码点切分 → `tapcanvas_flow_patch` → 输出写入回执，共5节点。`sourceGroupId` 必须来自当前调用者画布；每段默认120字符，允许显式 `triggerPayload.segmentCharLength`（1–10000），末段保留、不补写、不截断总篇幅、不限10节点。输出为普通 `kind=video` 的待生成节点，不含 Clip、Agent、Skill、知识库或媒体生成执行器，不生成付费媒体。工具执行沿用调用者 project/flow/chapter 权限与现有写入协议；最终聊天验收仍使用通用 `expectedDelivery -> deliveryEvidence -> deliveryVerification`，不能以工作流返回就冒充视频生成成功。发布 SQL 与启动代码从同一图定义生成，新增独立系统工作流，不覆盖已有用户工作流、版本、偏好、历史或资产。
...
- 一次 `request_user_input` 是不可拆分的单阶段输入合同，并且现在必须携带可机器验真的 `blockingBasis`：`contract_unresolved` 逐字引用当前已冻结 `UserIntentContract.unresolved` 中的一个或多个未决事实/授权，`delivery_reference_choice` 则逐个引用该合同 `referenceResolution=needs_user_choice` 的完整候选 ID 集合；两者都必须附事实证据引用。runtime 只比较这些结构化合同字段，不读取问题文案做语义判断。合同不存在、未决数组为空、引用集合不一致，或 Agent 只是因为 checkpoint/runId 缺失、担心重复生成、需要安全重启、需要自行规划创作/DAG 而询问时，当前工具动作会显式失败并把证据回灌同一链继续执行，不能把任务投影为 `needs_input`。章节画布入口已统一进入 Web 主对话，补充选项只使用主对话的同一组选项组件：每个 question 独立单选，但单项选择只更新本地草稿；只有当前 request 中全部 question 都有答案后，“确认并继续”才可用，并一次提交按 question 顺序排列的完整 `requestUserInputResponse`。前端不得把某一组选项的点击直接投影为 continuation，也不得为同一阶段发起多次续跑；这样一个阶段包含题材、结尾等多组决策时，agents-cli 只会在完整事实到齐后重新 claim 同一逻辑任务。
...
- Agent 配置对已装配工作流提供“更新并覆盖”：用户点击后依次检查当前保存版本并提交同一次签名 inspection token，直接替换当前 attachment 指针，保留原 scope、历史版本、执行与资产。已有同目标 coexist 关系继续保留，新增需选择的非阻断关系默认替换；blocking 与提交竞态仍显式失败。首次添加继续展示检查结果。共同工具或输出合同仅作为 info/acknowledge 的结构事实，不自动判定职责重叠或停用其他工作流；语义职责关系由 agents-cli 分析。
...
本地开发 compose 显式设置 `WORKFLOW_LOCAL_JAVASCRIPT_ENABLED=true`，只服务开发模式/Admin 工作流中的可信 JavaScript 节点。整图执行在无继承环境变量、无网络能力的独立 Node 子进程中运行，单次 5 秒超时；生产 compose 的 workflow-runtime-worker 同样显式开启，以执行系统一键成片v1的确定性拆分；可通过环境变量显式关闭，关闭时脚本节点报错。长文模板用该节点做换行归一化和段落结构整理；15 秒叙事单元数量与边界由后续 Agent 动态规划，再交给 collection split 与逐项提示词 Agent，固定字符数只用于用户显式要求的机械字长拆分，不得冒充语义拆分。
...
- 图定义：`src/modules/agents/system-one-click-workflow.definition.ts`。
- 数据发布：`src/modules/agents/system-one-click-workflow.ts`；启动时在同一事务内发布 `projects`、`flows`、不可变 `flow_versions` 和 `agent_capability_attachments(scope=all_users)`。身份冲突或版本漂移直接失败，重复启动不覆盖既有数据。
