...
<picture>
  ...
  <img alt="Seedance 2.x Skill OS — intent-first AI filmmaking. Route, verify, direct, deliver." src="assets/hero-dark.svg" width="100%">
</picture>

# Seedance 2.x Skill OS

**Direct the model. Don't micro-manage the frame.**

An agent that directs Seedance 2.0 and the verified Seedance 2.5 profile like a filmmaker — reading each scene before it writes the prompt.<br>Text, image, video, ordered storyboards, and reference to video with native audio, IP-safe rewrites, source-dated platform facts, and native reader paths for English, 中文, 日本語, and 한국어.

[![Version](https://img.shields.io/badge/version-6.7.0-E2A75E?style=flat-square&labelColor=14110B)](#changelog)
...
Seedance 2.x Skill OS is English-readable, but the v6 line gives Chinese, Japanese, and Korean readers first-class entry points, active example skills, and native prompt guidance. On verified surfaces that supply 2.0-style reference tokens, keep them exactly as written (`@Image1`, `@Video1`, `@Audio1`, `@图片1`, `@视频1`) in every language. For Seedance 2.5, consume the active structured surface tokens and limits instead of inheriting these examples.
...
Seedance 2.x Skill OS is a modular agent-skill package for directing verified Seedance 2.0 and Seedance 2.5 video generations. It is built around a simple principle: **direct the model, do not micro-manage the frame**.
...
This skill package turns Seedance 2.x work into a repeatable, profile-aware assistant workflow:
...
|---|---|---|
| “Use Seedance 2.5 for one 16–30 second scene or an ordered whole-board reference.” | [`seedance-25-longform-storyboard`](references/seedance-25-longform-storyboard.md) | A surface-gated 2.5 plan that keeps reference roles separate and does not inherit the 2.0 15-second ceiling. |
...
As of 2026-08-03, ByteDance's official Seedance 2.5 page describes up to 30 seconds in one generation, two extensions, and stronger reference/editing controls. The current TapCanvas ARK catalog exposes a surface-specific `doubao-seedance-2.5` profile; live IDs, limits, pricing, request fields, access, and regional behavior must still be rechecked.
...
- [`model-name-map.md`](references/model-name-map.md) prevents `Seedance 2.5`, `Seedance 2.0`, Fast/Mini variants, `Seedance V2`, and ambiguous Pro labels from being mixed together.
...
| Reference | Purpose |
|---|---|
| [`seedance-25-longform-storyboard.md`](references/seedance-25-longform-storyboard.md) | Evidence-ranked Seedance 2.5 profile for 30-second storytelling, whole-board temporal references, long dialogue/action coverage, and the current TapCanvas executor boundary. |
...
| [`model-name-map.md`](references/model-name-map.md) | Seedance 2.0/2.5 naming, surface IDs, Fast/Mini variants, and ambiguous V2/Pro caveats. |
...
The TapCanvas integration also carries nine focused Seedance 2.5 regression cases in [`evals/evals.json`](evals/evals.json): verified 30-second whole-board planning, dialogue/prop continuity, unknown-model handling, rejection of template quotas, the current 1–3 state executor boundary, user-named-but-unverified 2.5, unknown speaking-rate capacity, 2.0 token leakage, and unsupported Fast/Standard tier leakage.
...
Current release: **v6.7.0** (2026-08-03).

- Adds a public-source-verified Seedance 2.5 model profile and keeps 2.0/2.5 duration rules separate.
- Distills two separate local n=1 observations (one dramatic, one whole-board combat) into evidence-ranked methods without loading `assets/` at runtime or treating either case as repeated reliability.
- Removes inherited fixed-cut, fixed-action, minimum-character, mandatory-confirmation, and silent-model-fallback assumptions from the new profile.
- Adds nine regression cases for 30-second whole-board prompting, dialogue capacity, ambiguous model selection, template quota rejection, the current TapCanvas 1–3 state execution boundary, explicitly named but unverified 2.5, unknown speaking-rate capacity, 2.0 reference-token leakage, and unsupported Fast/Standard tier leakage.
