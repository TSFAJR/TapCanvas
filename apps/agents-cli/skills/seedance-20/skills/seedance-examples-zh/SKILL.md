...
Use examples as patterns, not as content to copy blindly. Chinese prompts should stay compact, concrete, and production-oriented. Literal `@Image1`, `@Video1`, `@Audio1`, `@图片1`, `@视频1`, and `@音频1` strings below are Seedance 2.0 surface examples: preserve them only when the user or active surface supplied those exact tokens. For Seedance 2.5 without token mapping, substitute neutral asset-role names and bind execution through the structured reference fields.
