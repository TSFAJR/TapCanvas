...
Surface reference contract: literal `@Video1` examples below are Seedance 2.0 surface syntax. Emit only exact tokens supplied by the user or active structured surface; for Seedance 2.5 without token mapping, name the motion-reference role neutrally and bind it through the active structured fields.
