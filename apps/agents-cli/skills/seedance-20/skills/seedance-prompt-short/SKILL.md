...
description: "This skill should be used when the user asks for a compact Seedance 2.x prompt, short Chinese prompt, prompt compression, 30-100 word output, or removal of unnecessary prompt language. Reference tokens must come from the active structured surface contract or the user's exact supplied tags."
license: MIT
user-invocable: true
tags:
  - prompt-compression
...
Surface reference contract: literal `@Image1`/`@Video1`/`@Audio1` strings in historical examples are Seedance 2.0 surface syntax. Use only exact tokens supplied by the user or active structured surface. For Seedance 2.5 without token mapping, use neutral asset-role names in a conditional plan or the active structured reference fields during execution; never invent a tag.
...
| I2V | `[exact active image-reference token or neutral asset name] is preserved; only [motion/light/camera] changes. Camera: [one move]. Sound: [cue]. Constraint: [what must not change].` |
| V2V | `[exact active video-reference token or neutral asset name] controls [motion/camera/timing] only; new subject [anchor]. [Action]. Do not transfer [identity/scene/logo].` |
| Chinese | `[活动 surface 提供的精确图像引用 token，或中性资产名]为参考，严格保持[主体]不变；仅加入[动作/光线/镜头]。声音：[提示]。` |
