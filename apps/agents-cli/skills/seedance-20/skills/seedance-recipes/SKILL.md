...
Surface reference contract: every literal `@Image1`/`@Image2`/`@Video1`/`@Audio1` recipe below is a Seedance 2.0 surface example. Substitute only exact tokens supplied by the user or active structured surface; on Seedance 2.5 without token mapping, use neutral asset-role names and never emit the example tags.
