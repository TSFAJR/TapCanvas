...
Surface reference contract: reference tags mentioned below are preservation examples, not tags to invent. Preserve an exact user-supplied or active-surface token byte for byte; for Seedance 2.5 without token mapping, record neutral asset roles and defer executable field binding to the structured surface contract.
