...
English is the default prompting language and fails in two ways at once: slop (empty evaluation words that add tokens and no signal) and false positives (vague threat-adjacent wording that trips the heaviest moderation surface). The cure for both is the same: concrete production English. Literal `@Image1`, `@Video1`, and `@Audio1` strings below are Seedance 2.0 surface examples. Preserve them exactly only when the user or active surface supplied those tokens; for Seedance 2.5 without token mapping, use neutral asset-role names and the structured reference fields.
