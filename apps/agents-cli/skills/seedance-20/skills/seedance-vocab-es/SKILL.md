...
Use Spanish cinematic vocabulary when the user asks for Spanish prompts, bilingual delivery, or compact translation of camera, lighting, action, VFX, audio, and production constraints. Literal `@Image1`, `@Video1`, and `@Audio1` strings below are Seedance 2.0 surface examples. Preserve them exactly only when the user or active surface supplied those tokens; for Seedance 2.5 without token mapping, use neutral asset-role names and the structured reference fields.
