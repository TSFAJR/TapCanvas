...
Use Japanese examples as native prompt patterns, not translated English templates. The literal `@Image1`, `@Image2`, `@Video1`, and `@Audio1` strings below are Seedance 2.0 surface examples: preserve them only when the user or active surface supplied those exact tokens. For Seedance 2.5 without token mapping, substitute neutral asset-role names and bind execution through the structured reference fields.
