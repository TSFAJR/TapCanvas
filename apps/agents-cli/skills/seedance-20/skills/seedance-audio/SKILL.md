...
Surface reference contract: literal `@Audio1`, `@Video1`, and `@Image1` strings below are Seedance 2.0 surface examples. Emit only exact tokens supplied by the user or active structured surface; on Seedance 2.5 without token mapping, use the structured reference fields or neutral asset-role names and never invent tags.
