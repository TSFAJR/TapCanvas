...
Surface reference contract: literal `@Image1` examples below are Seedance 2.0 surface syntax, not a required character-tag form. Emit only exact tokens supplied by the user or active structured surface; otherwise use stable neutral character names and the active structured reference fields.
