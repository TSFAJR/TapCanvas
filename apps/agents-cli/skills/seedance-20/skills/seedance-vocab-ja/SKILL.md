...
Use Japanese cinematic vocabulary when the user asks for Japanese prompt wording, bilingual delivery, compact translation, or production vocabulary for camera, lighting, motion, VFX, and audio. Literal `@Image1`, `@Video1`, and `@Audio1` strings below are Seedance 2.0 surface examples. Preserve them exactly only when the user or active surface supplied those tokens; for Seedance 2.5 without token mapping, use neutral asset-role names and the structured reference fields.
