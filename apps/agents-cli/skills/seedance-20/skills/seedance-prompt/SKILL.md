...
description: "This skill should be used when the user asks to write, improve, translate, compress, or debug a Seedance 2.x video prompt; mentions T2V, I2V, V2V, R2V, camera direction, prompt quality, or provides reference assets for a production-ready prompt. Model profile, duration, request fields, and reference tokens must come from the active structured surface contract."
license: MIT
user-invocable: true
tags:
  - prompt-engineering
...
Surface reference contract: every literal `@Image1`/`@Video1`/`@Audio1` example in this skill or a loaded reference is a Seedance 2.0 surface example. Emit only the exact token supplied by the user or active structured surface. For Seedance 2.5 without a documented prompt-token mapping, bind the active structured reference fields during execution or use neutral asset-role names in a conditional plan; never invent tags or counts.
...
Choose the mode before drafting. **T2V** needs subject, action, scene, camera, light, style, and constraints because nothing is visible yet. **I2V** starts from the image reference bound by the active surface and adds only motion, time, camera, lighting transition, audio, and preservation. **V2V** maps the bound video reference to source clip, camera move, action rhythm, blocking, edit target, or extension anchor rather than accidentally transferring identity. **R2V** lists every reference role and states what must not transfer. **FLF2V** uses the active surface's distinct first-frame and last-frame roles, then describes only the continuous transition; do not invent those roles or their tokens when the surface does not expose them.
...
| I2V | Preserve visible identity; add motion. | Re-describing the image until the product or face drifts. | Say the exact bound image reference must be preserved; add only dynamic changes. |
...
| FLF2V | Move from first frame to last frame. | Treating the last frame as vague mood instead of endpoint. | State that the exact bound last-frame asset is the final visual target. |
| Edit | Preserve the source clip while changing one layer. | Rewriting the whole scene and losing continuity. | Bind the exact source-video reference and say `change only...`. |
