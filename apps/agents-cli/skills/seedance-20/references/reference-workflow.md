...
The asset-role method in this file applies across Seedance 2.x, but the `@` tag names and upload-count examples below are verified Seedance 2.0 surface syntax, not a family-wide contract. For Seedance 2.5, consume the active structured surface/model reference schema and preserve the exact tokens it supplies. Never inherit the 2.0 image/video/audio counts, request fields, or tag spellings when the 2.5 surface has not confirmed them.

## Seedance 2.0 Reference Tag Syntax

On verified Seedance 2.0 surfaces that expose this picker, uploaded assets bind through an `@`-mention typed directly in the prompt. Type `@` in the prompt field to pick an uploaded file, or preserve the exact supplied tag inline. Tags are assigned by type and upload order on those surfaces.
