...
2. Treat beats as editorial jobs first. Assign the theoretical minimum number of Seedance clips allowed by the active duration contract, then place several labeled shots inside a clip when the scene is fast or dense. Only assign one clip per beat when a real production boundary makes the beat independent; a beat count alone is not a billing or generation boundary.
