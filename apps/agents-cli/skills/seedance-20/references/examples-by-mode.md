...
Surface reference contract: literal `@Image1`/`@Image2`/`@Video1`/`@Audio1` and Chinese equivalents in these examples are Seedance 2.0 surface syntax. Preserve them only when the user or active surface supplied those exact tokens. For Seedance 2.5 without token mapping, substitute neutral asset-role names and bind execution through the structured reference fields.
