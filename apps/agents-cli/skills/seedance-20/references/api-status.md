# Seedance 2.x API and Platform Status

last_verified: 2026-08-03
confidence: public-source and current-repository snapshot as of the verification date; the Seedance 2.5 public model profile and TapCanvas ARK contract were verified 2026-08-03, while older per-section dates still apply to Seedance 2.0 surfaces; not a guarantee of access, pricing, model IDs, upload limits, authorization behavior, or regional availability on every surface

## Seedance 2.5 Profile *(verified 2026-08-03)*

### Confirmed public model facts

- ByteDance's official Seedance 2.5 page describes an audio-video joint generation model built for 30-second storytelling.
- The official page says one generation can be up to 30 seconds and can be extended twice.
- It describes more precise reference-video understanding, stronger audio/visual editing responses, white-model control, green-screen editing, professional camera movement, and performance blocking.

These are broad model claims, not a universal API schema or entitlement promise. The public page does not by itself establish a provider's model ID, exact request fields, reference counts, pricing, resolution, region, or account access.

Primary source: https://seed.bytedance.com/en/seedance2_5.

### Current TapCanvas ARK contract — internal-contract and surface-specific

The checked-in ARK registration maps `doubao-seedance-2.5` to `doubao-seedance-2-5-260628`. Its current structured model catalog exposes 4–30 second duration choices, 480p/720p, multimodal image/video/audio references, audio-only reference, first/last frame, editing, extension, ultra-long capability metadata, timestamp prompting, and native audio. It also declares current per-surface reference and duration maxima.

Treat those values as active TapCanvas/ARK implementation facts that must still be refreshed from the live model catalog before execution. Do not copy them to Dreamina, Jimeng, BytePlus, Runway, fal, or another provider, and do not use a prompt keyword to select the model.

Repository evidence: `apps/new-api/patches/2026-07-31/005-add-seedance-2-5.sql`, `apps/hono-api/src/modules/new-api-models/new-api-models.service.test.ts`, and `apps/web/_test/unit/modelCatalogMeta.test.ts`.

### Local long-form observations — n=1 per workflow shape

- Dramatic pair: one 30.08-second output built from a single scene/character reference and a multi-shot dramatic prompt.
- Combat pair: one 30.08-second output built from an intact 4×4 ordered storyboard plus role/scene constraints.

Each exact workflow shape has one observation. The combat pair supports testing a whole-board temporal-reference workflow, but n=1 does not establish repeated reliability, an exact cut-count, timing, lip-sync, or universal transport behavior. Load `seedance-25-longform-storyboard.md` for the distilled method and current TapCanvas executor boundary.

## Seedance 2.0 — Confirmed From Public Sources
...
**No dedicated extend endpoint** — extend is a Dreamina-app feature. To continue a clip on fal, prefer reference-to-video with the previous clip as a video reference (keeps motion and audio context). Chaining image-to-video from the previous clip's last frame is a distinct, explicit alternative only when the surface supports it and the user-facing continuity trade-off is stated; never switch workflows silently.
...
> As of 2026-08-03, ByteDance publicly describes Seedance 2.5 as an audio-video joint generation model for up to 30 seconds of storytelling in one generation, with two extensions and stronger reference/editing controls. Seedance 2.0 remains a separate 4–15 second profile with documented mixed text, image, audio, and video references. Access, model IDs, pricing, request fields, input limits, resolution, region, face/reference handling, and authorization remain surface-specific and must be rechecked before production use.

## Model Naming Rule

- Use `Seedance 2.5` only for the official 30-second model profile or a surface that explicitly exposes it; do not treat it as an alias for 2.0.
- Use `Seedance 2.0` for the earlier official 4–15 second model profile.
...
- Use `doubao-seedance-2.5` and upstream `doubao-seedance-2-5-260628` only for the current TapCanvas/Volcengine ARK contract after a live recheck.
