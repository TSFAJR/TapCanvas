...
Use this reference for Chinese Seedance prompt wording, role binding, and compact prompt compression. Literal `@Image1`, `@Video1`, `@Audio1`, `@图片1`, `@视频1`, and `@音频1` strings below are versioned Seedance 2.0 surface examples: preserve them only when the user or active surface supplied those exact tokens. On Seedance 2.5 without token mapping, substitute neutral asset-role names and use the structured reference fields for execution.
...
社区常用的长提示词骨架（即梦/Dreamina Seedance 2.0 surface，约 8 秒以上时使用；field-observed）。仅当活动 surface 提供相同 token 时，才把中性引用槽替换为对应 `@Image1` 等精确标签：

```
【风格】[媒介、质感、色调，一句话]
【时间轴】0-3s：[画面+镜头+音效]；3-6s：[画面+镜头+音效]；6-10s：[画面+镜头+音效]
【声音】[对白/环境声/音效/无配乐]
【参考】[身份图像引用] 锁定主体身份；[运镜视频引用] 仅参考运镜；[节奏音频引用] 仅参考节奏
```
