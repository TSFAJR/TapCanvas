...
Seedance 2.x capability claims must separate the exact model profile from the product surface. A feature can be true for Seedance 2.0 or 2.5 while still being gated, unavailable, renamed, priced differently, or policy-limited on a specific surface.
...
|---|---|---|---|
| ByteDance Seedance 2.5 official model page | official | Broad 2.5 capability framing | Verified 2026-08-03 for up-to-30-second single generation, two extensions, stronger reference/editing behavior, white-model control, and green-screen editing. It is not an API schema, model-ID list, price table, or entitlement guarantee. |
| TapCanvas / Volcengine ARK Seedance 2.5 profile | internal-contract structured surface evidence | Current repository execution planning | Current mapping is `doubao-seedance-2.5` → `doubao-seedance-2-5-260628` with dynamic 4–30 second and multimodal capability metadata. Refresh the live catalog before execution and do not generalize its limits or fields to another surface. |
...
Verified 2026-06-20 across every Seedance 2.0 developer surface checked here (fal, Replicate, Volcengine Ark, BytePlus ModelArk, Atlas Cloud, Runway, WaveSpeed, Pollo, EvoLink, OpenRouter, Kie.ai, PiAPI, LaoZhang, Runware, ModelsLab, AI/ML API, MuAPI, SeeGen, Segmind): video generation uses an async job - submit a task, get an id, poll until ready, fetch the URL. The current TapCanvas ARK Seedance 2.5 integration remains on the repository's asynchronous video route, but another 2.5 surface may differ. Never transfer a 2.0 request shape or an LLM/chat request shape to 2.5 without live surface evidence.
