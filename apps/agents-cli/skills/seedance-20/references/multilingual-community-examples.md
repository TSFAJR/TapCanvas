...
Surface reference contract: literal `@Image1`/`@Image2`/`@Video1`/`@Audio1` and localized equivalents in these practitioner examples are versioned Seedance 2.0 surface syntax. Preserve them only when the user or active surface supplied those exact tokens. For Seedance 2.5 without token mapping, substitute neutral asset-role names and use the structured reference fields during execution.
