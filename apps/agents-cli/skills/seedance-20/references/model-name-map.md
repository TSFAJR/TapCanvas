...
Use this file when a user says "Seedance 2.5", "Seedance Pro", "Seedance V2", "Seedance V2 Mini", "Seed2.0 Pro", or wrapper-specific model names.

## Canonical Names

| Name | Meaning | Guidance |
|---|---|---|
| Seedance 2.5 | ByteDance's official 30-second audio-video generation profile | A user label may activate conditional 2.5 planning only; executable activation still requires structured model/surface facts. The public page states up to 30 seconds and two extensions; recheck the active surface before using model IDs, limits, pricing, or request fields. |
| `doubao-seedance-2.5` / `doubao-seedance-2-5-260628` | Current TapCanvas public key and Volcengine ARK upstream mapping | Internal, surface-specific implementation facts verified 2026-08-03. Consume the live model catalog before execution; do not export these names to another provider. |
| Seedance 2.0 | ByteDance's earlier 4–15 second video generation profile | Correct public name for 2.0 work. Do not use it as a family-wide default after 2.5 has been selected. |
...
| Seedance V2 | Ambiguous community shorthand | Do not silently normalize it to 2.0 now that 2.5 is a distinct public profile. Resolve the selected model/surface from structured context or ask which version the user means before execution. If the user says `Seedance V2 Mini`, normalize to Seedance 2.0 Mini only after checking the active surface. |
...
If the user says "Seedance 2.5", answer:

`I will use the Seedance 2.5 profile only if the selected surface exposes it. ByteDance publicly describes up to 30 seconds in one generation and two extensions; the exact model ID, duration choices, reference limits, resolution, pricing, and access are surface-specific and need a live check before execution.`
